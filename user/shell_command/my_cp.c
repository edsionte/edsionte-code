...
void cp_single(char *original_src_path,char* original_dest_path)
{
	...
	if(param_p)
	{
		if(fchmod(fdwt,src_buf.st_mode)==-1)
        	{
			perror("fchmod");
              		exit(1);
         	}
        	//set file's user id and group id
         	if(fchown(fdwt,src_buf.st_uid,src_buf.st_gid)==-1)
         	{
			perror("fchown");
              		exit(1);
         	}
...
void cp_directory(char* src_path,char* dest_path)
{
	...
	else
	{
		...
		if(stat(src_path,&src_buf)!=0)
		{
			printf("my_cp:在cp_directory函数中：获取源文件\"%s\"属性时失误\n",src_path);
		}
	...
	char original_dest_path[PATH_MAX+1];
	strcpy(original_dest_path,dest_path);
