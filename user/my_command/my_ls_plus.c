...
int g_maxlen;
int if_i;//print index number of each file
int if_t;//sort files by st_mtime
int if_A;//do not list . and ..
int if_B;//do not list the file ending with ~
int if_d;
...
void display_single(struct stat,char*);
void display_attribute(struct stat,char*);
void display_dir(int,char*);
void sort_by_mtime(char filename[][PATH_MAX+1],int count);
...
void convert_time(char mtime[],char buf_time[])
{
	int i,k;

	for(i=0;i<4;i++)
	{
		mtime[i]=buf_time[20+i];
	}
	mtime[i++]='-';
	if(buf_time[4]=='J')
	{
		if(buf_time[5]=='a'&&buf_time[6]=='n')
		{
			mtime[i++]='0';
			mtime[i++]='1';
		}
		else if(buf_time[5]=='u'&&buf_time[6]=='n')
		{
			mtime[i++]='0';
			mtime[i++]='6';
		}
		else if(buf_time[5]=='u'&&buf_time[6]=='l')
		{
			mtime[i++]='0';
			mtime[i++]='7';
		}
	}
	else if(buf_time[4]=='M'&&buf_time[6]=='r')
	{
		mtime[i++]='0';
		mtime[i++]='3';
	}
	else if(buf_time[4]=='M'&&buf_time[6]=='y')
	{
		mtime[i++]='0';
		mtime[i++]='5';
	}
	else if(buf_time[4]=='A'&&buf_time[5]=='p')
	{
		mtime[i++]='0';
		mtime[i++]='4';
	}
	else if(buf_time[4]=='A'&&buf_time[5]=='u')
	{
		mtime[i++]='0';
		mtime[i++]='8';
	}
	else if(buf_time[4]=='F')
	{
		mtime[i++]='0';
		mtime[i++]='2';
	}
	else if(buf_time[4]=='S')
	{
		mtime[i++]='0';
		mtime[i++]='9';
	}
	else if(buf_time[4]=='O')
	{
		mtime[i++]='1';
		mtime[i++]='0';
	}
	else if(buf_time[4]=='N')
	{
		mtime[i++]='1';
		mtime[i++]='1';
	}
	else if(buf_time[4]=='D')
	{
		mtime[i++]='1';
		mtime[i++]='2';
	}
	...
	for(k=0;k<8;k++)
	{
		mtime[i++]=buf_time[11+k];
	}
	mtime[i]='\0';

}

//sort file by st_mtime of each files
void sort_by_mtime(char filename[][PATH_MAX+1],int count)
{
	char mtime[256][20],temp[PATH_MAX+1];
	int i,j,k;
	char buf_time[50];
	struct stat buf;
	for(i=0;i<count;i++)
...
void display_dir(int flag_param,char *path)
{
	...
	if(if_t)
	{
		//sort the file by st_mtime of each file
		sort_by_mtime(filename,count);
	}
	else
	{
	...
	if(if_d)
	{
		...
		if(flag_param==PARAM_L||flag_param==PARAM_L+PARAM_A)
	...
	if(flag_param==PARAM_A||flag_param==PARAM_NONE||if_d)
	{
		printf("\n");
	}
